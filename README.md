...
# WhatsApp Bot con OpenAI y Google Calendar

Bot de WhatsApp que utiliza OpenAI para responder mensajes y ofrece integración con Google Calendar para la gestión de citas.

## Características

- Procesamiento de mensajes con OpenAI Assistant
- Integración con Google Calendar para:
  - Consultar disponibilidad de horarios
  - Crear citas en el calendario

## Uso de la Integración de Google Calendar

El bot ahora puede consultar al calendario y agendar citas directamente desde las conversaciones en WhatsApp. 

### Consulta de disponibilidad

Cuando un cliente pregunta por horarios disponibles, el asistente puede consultar automáticamente el calendario de Google y mostrar los horarios libres.

Ejemplo:
```
Cliente: "¿Qué horarios tienen disponibles para mañana?"
Bot: [consulta el calendario] "Para mañana tenemos los siguientes horarios disponibles: 10:00-11:00, 15:00-16:00..."
```

### Programación de citas

Si un cliente desea agendar una cita, el asistente puede verificar la disponibilidad y crear el evento directamente en el calendario.

Ejemplo:
```
Cliente: "Quiero agendar una cita para mañana a las 3pm"
Bot: [verifica disponibilidad] "Perfecto, he agendado su cita para mañana a las 3:00pm. Se ha enviado la confirmación a su correo."
```

## Configuración del Asistente OpenAI

Para habilitar la funcionalidad de Google Calendar, el asistente de OpenAI debe configurarse para utilizar function calling. Las funciones disponibles son:

- `check_calendar_availability`: Consulta los horarios disponibles
- `create_calendar_event`: Crea eventos en el calendario
...
Esta URL debe apuntar a donde esté alojado tu dashboard de Next.js que contiene las APIs de Google Calendar.

## Instrucciones para Desarrolladores

Para habilitar esta funcionalidad en el asistente de OpenAI, puedes actualizar las instrucciones del asistente con la siguiente información:

```
Este asistente tiene la capacidad de consultar el calendario del negocio y agendar citas. Cuando un cliente pregunte sobre horarios disponibles, utiliza la función check_calendar_availability para consultar los horarios libres. Si el cliente desea programar una cita, verifica primero la disponibilidad antes de crear el evento con create_calendar_event.
```
